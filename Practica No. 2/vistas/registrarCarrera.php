...
<div class="container">
    <div class="jumbotron">
        <h2>Formulario registro de Carreras</h2>
    </div>
            ...
            <?php } ?>
                ...
                <div class="form-group">
                    ...
                    <?php foreach($query as $data2): ?>
                        <!-- aqui se hace una validacion para agregar el atributo selected a la etiqueta option para tener seleccionada la opcion de la universidad actual del registro que se va a editar(En caso de editar el registro) -->
                        <option value="<?php echo $data2["id"]; ?>"  <?php if(isset($data['id'])){ if($data["id_universidad"]==$data2["id"]){ echo "selected"; } } ?> > <?php echo $data2["nombre"]; ?> </option>
                    <?php endforeach; ?>
                    </select>
                    </div>
                </div>
