...
{
    //funcion que retorna los valores de los pedidos realizados por el usuario actual, junto aqui se consulta la empresa del producto y su imagen.
    public function index(){
    	return DB::table('pedidos')->join('productos','productos.id','=','pedidos.id_producto')->join('users','users.id','=','pedidos.id_usuario')->join('empresas','empresas.id','=','productos.id_empresa')->join('imagenes_productos','imagenes_productos.id_producto','=','productos.id')->select('pedidos.*','pedidos.id as id_pedido','empresas.nombre as nombre_empresa','productos.*','imagenes_productos.imagen')->where('pedidos.id_usuario', '=', Auth::user()->id )->get();
    }
